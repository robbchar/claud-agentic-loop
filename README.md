...
### Project discovery and file writing

When you run the swarm, it automatically scans the output directory before
generating anything. This lets the Dev agent know the framework, existing file
structure, and current file contents — so it generates code that fits the
project rather than starting from scratch.

After the Reviewer approves, the generated files are written to disk
automatically. The Dev agent wraps each file in a separator:

```
--- FILE: src/components/DeckBuilder.tsx ---
<file contents>
...
The writer creates any missing directories and overwrites existing files
silently (the Dev agent has already read their contents and decided what to
write).

**Running from inside your project (recommended):**

```bash
cd ../my-project
python ../claud-agentic-loop/main.py --architecture docs/ARCHITECTURE.md --tasks docs/TASKS.md
```

Files are written into `my-project/` automatically.

**Or point at a project from anywhere:**

```bash
python main.py --architecture ../my-project/docs/ARCHITECTURE.md \
               --tasks ../my-project/docs/TASKS.md \
               --output-dir ../my-project
```

**Large file prompt:**

If the scout finds a file over 10 KB it will pause and ask:

```
  Large file: src/legacy/BigComponent.tsx (14.3 KB)
  Files over 10 KB are a code smell — consider breaking this down first.
  [s] Skip  — Dev agent won't see its contents (listed in tree only)
  [q] Quit  — exit now and refactor the file first (recommended)
  Choice [s/q]:
```

This is intentional — very large files are usually a sign something should be
split up. If it's justified (e.g. a lookup table), choose `s` to skip it.

### Dry-run

Verify file loading and project discovery without making any Claude API calls:
...
```
[spec] Loaded architecture doc: docs/ARCHITECTURE.md (1842 chars)
[spec] Loaded tasks doc: docs/TASKS.md (743 chars)

[scout] Scanning project: /Users/me/my-project
[scout] Framework : Next.js (detected from next.config.ts)

--- DRY RUN STATE ---
feature_request:  '\nBuild a Python function...'
architecture:     1842 chars loaded
tasks_doc:        743 chars loaded
project_context:  3201 chars loaded
output_dir:       /Users/me/my-project
No API calls made.
```

Works with any combination of flags — useful for verifying setup before
...
test_main.py         — CLI flags, file loading, --dry-run, --output-dir, error exits
test_scout.py        — Framework detection, file tree, large-file prompting, ignore rules
test_writer.py       — FILE block parsing, multi-file writes, directory creation, overwrite
```
